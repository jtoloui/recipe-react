...
export const Avatar = ({ isOpen, setIsOpen, isSideMenuVersion = false }: AvatarProps) => {
	...
	const RenderMenuItems = () => {
		return (<>
			{menuItems.map(item => (
				<Link to={item.to} key={item.text.replace(' ', '-')} onClick={item.onClick} className="block px-4 py-3 text-sm text-gray-600 capitalize transition-colors duration-300 transform hover:bg-gray-100">
					{item.text}
				</Link>
			))}
		</>
